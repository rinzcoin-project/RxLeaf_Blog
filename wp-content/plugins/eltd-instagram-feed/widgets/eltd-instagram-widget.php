...
class ElatedInstagramWidget extends WP_Widget {
	...
	public function __construct() {
		parent::__construct(
			...
			array( 'description' => __( 'Display instagram images', 'eltd_instagram_feed' ) )
		);
	...
	public function widget($args, $instance) {
		...
		$type_of_widget = $type_of_widget == '' ? 'grid' : $type_of_widget;

		if(is_array($images_array) && count($images_array)) { ?>
			<ul class="eltd-instagram-feed clearfix eltd-col-<?php echo $number_of_cols; ?> <?php echo $type_of_widget ?>">
				<?php
				foreach ($images_array as $image) { ?>
					<li>
						<a href="<?php echo esc_url($instagram_api->getHelper()->getImageLink($image)); ?>" target="_blank">
							<?php echo flow_elated_kses_img($instagram_api->getHelper()->getImageHTML($image, $image_size)); ?>
						</a>
					</li>
				<?php } ?>
			</ul>
