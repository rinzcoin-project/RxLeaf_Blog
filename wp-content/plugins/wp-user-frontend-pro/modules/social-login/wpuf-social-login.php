...
Class WPUF_Social_Login {
    ...
    public function __construct() {
        if ( !class_exists( 'Hybrid_Auth' ) ) {
            if ( class_exists( 'WeDevs_Dokan' ) && class_exists( 'Dokan_Pro' ) && ( $_GET['wpuf_reg'] != 'instagram' || $_GET['wpuf_reg'] != 'pinterest' ) ) {
        ...
        $this->base_url = site_url() . '/account/';
    ...
    **/
    ...
    private function get_providers_config() {
        ...
        if ( $instagram_id != '' && $instagram_secret != '' ) {
            $config['providers']['Instagram']['keys']['id']    = $instagram_id;
            $config['providers']['Instagram']['keys']['secret'] = $instagram_secret;
        }
        //Pinterest config from admin
        $pinterest_id     = wpuf_get_option( 'pinterest_app_id', 'wpuf_social_api' );
        $pinterest_secret = wpuf_get_option( 'pinterest_app_secret', 'wpuf_social_api' );
        if ( $pinterest_id != '' && $pinterest_secret != '' ) {
            $config['providers']['Pinterest']['keys']['id']    = $pinterest_id;
            $config['providers']['Pinterest']['keys']['secret'] = $pinterest_secret;
        }
        ...
        $social_settings_fields = array(
            'wpuf_social_api' => array(
                ...
                'facebook_app_url'    => array(
                    ...
                    'type'  => 'html',
                    'desc'  => "<input class='regular-text' type='text' disabled value=" . $this->base_url . '?wpuf_reg=facebook&form_id={{form_id}}&hauth_done=Facebook' . '>',
                ),
                ...
                'google_app_url'      => array(
                    ...
                    'label' => __( 'Redirect URI', 'wpuf-pro' ),
                    'type'  => 'html',
                    'desc'  => "<input class='regular-text' type='text' disabled value=" . $this->base_url . '?wpuf_reg=google&form_id={{form_id}}&hauth_done=Google' . '>',
                ),
                'google_app_id'       => array(
                    'name'  => 'google_app_id',
                ...
                'linkedin_app_url'    => array(
                    ...
                    'type'  => 'html',
                    'desc'  => "<input class='regular-text' type='text' disabled value=" . $this->base_url . '?wpuf_reg=linkedin&form_id={{form_id}}&hauth_done=LinkedIn' . '>',
                ),
                ...
                'instagram_app_url'    => array(
                    ...
                    'label' => __( 'Redirect URI', 'wpuf-pro' ),
                    'type'  => 'html',
                    'desc'  => "<input class='regular-text' type='text' disabled value=" . $this->base_url . '?wpuf_reg=instagram&form_id={{form_id}}&hauth_done=Instagram' . '>',
                ),
                ...
                'pinterest_app_label'  => array(
                    'name'  => 'pinterest_app_label',
                    'label' => __( 'Pinterest App Settings', 'wpuf-pro' ),
                    'type'  => 'html',
                    'desc'  => '<a target="_blank" href="https://developers.pinterest.com/apps/">' . __( 'Create an App', 'wpuf-pro' ) . '</a> if you don\'t have one and fill App ID and App Secret below.  Also replace the {{form_id}} in Redirect URI with the registration form id where you are using Social Login.',
                ),
                'pinterest_app_url'    => array(
                    'name'  => 'pinterest_app_url',
                    'label' => __( 'Redirect URI', 'wpuf-pro' ),
                    'type'  => 'html',
                    'desc'  => "<input class='regular-text' type='text' disabled value=" . preg_replace("/^http:/i", "https:", $this->base_url) . '?wpuf_reg=pinterest&form_id={{form_id}}&hauth_done=Pinterest' . '>',
                ),
                'pinterest_app_id'     => array(
                    'name'  => 'pinterest_app_id',
                    'label' => __( 'App ID', 'wpuf-pro' ),
                    'type'  => 'text',
                ),
                'pinterest_app_secret' => array(
    ...
    public function render_social_logins() {
        ...
        $redirect_uri = preg_replace( "/^http:/i", "https:", $this->base_url );
            ...
            <ul class="jssocials-shares">
                <?php foreach ( $providers as $provider ) : ?>
                    <li class="jssocials-share jssocials-share-<?php echo $provider ?>">
                        <a href="<?php echo add_query_arg( array( 'wpuf_reg' => $provider, 'form_id' => $f_id ), $this->base_url ); ?>" class="jssocials-share-link">
