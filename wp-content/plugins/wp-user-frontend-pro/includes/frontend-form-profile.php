...
class WPUF_Frontend_Form_Profile extends WPUF_Render_Form {
    ...
    */
    public function resend_activation_email() {
        ...
        if ( !$user->id ) {
            wpuf()->login->add_error( 'User not found' );
            return;
        }
        //check if activated
        if ( $user->is_verified() ) {
            wpuf()->login->add_message( 'User is verified already' );
            return;
        }

        $this->send_verification_mail( $user->id, $user->user->user_email );

        wpuf()->login->add_message( 'Email Activation link is sent, please check your email.' );
